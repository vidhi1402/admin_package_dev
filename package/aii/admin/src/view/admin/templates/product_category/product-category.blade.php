...
@section('content')
            ...
            <section class="panel">
               {{-- <header class="panel-heading">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">--}}
                        @if(Request::route()->getName() == 'admin.product_category.index')
                            @include('admin::templates.product_category.product-category-content')
                        @else
                            @include('admin::templates.product_category.product-category-update')
                        @endif
                   {{-- </div>
                </div>--}}
            </section>
    ...
    @include('admin::templates.product_category.product-category-info-table')
    ...
    <script type="text/javascript">
        ...
        function changeStatus(this1,id)
        {
            if (confirm('STATUS cahnge?')) {
                ...
                var API_URL = "{{ route('admin.product_category.change_status') }}";
                $.ajax({
                    url: API_URL,
                    type: 'POST',
